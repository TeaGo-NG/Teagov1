<?php
include("functions/init.php");

?>
<!DOCTYPE html>
<html lang="en">

<head>

    <title>TeaGo COSE - Forgot Password</title>

    <meta charset="utf-8">
    <meta name="title" content="TeaGo COSE - Forgot Password">
    <meta content='width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0' name='viewport' />

    <!--  Social tags      -->
    <meta name="keywords" content="TeaGo, Teago NG, COSE">
    <meta name="description" content="TeaGo COSE - Forgot Password">

    <!-- Schema.org markup for Google+ -->
    <meta itemprop="name" content="TeaGo COSE - Forgot Password">
    <meta itemprop="description" content="TeaGo COSE - Forgot Password">

    <meta itemprop="image" content="https://cose.teagonig.com/home/assets/images/logo.png">

    <!-- OG Meta Tags to improve the way the post looks when you share the page on Facebook, Twitter, LinkedIn -->
    <meta name="description" content="TeaGo COSE - Forgot Password">
    <meta name="keywords" content="TeaGo COSE - Forgot Password">
    <meta property="og:title" content="TeaGo COSE - Forgot Password" />
    <meta property="og:image" content="https://cose.teagonig.com/home/assets/images/logo.png" />
    <meta property="og:url" content="https://cose.teagonig.com/home/assets/images/logo.png" />
    <meta property="og:site_name" content="TeaGo COSE - Forgot Password" />
    <meta property="og:description" content="TeaGo COSE - Forgot Password" />
    <meta name="theme-color" content="#be1e2d">

    <!-- ===============================================-->
    <!--    Favicons-->
    <!-- ===============================================-->
    <link rel="apple-touch-icon" sizes="180x180" href="home/assets/images/logo.png">
    <link rel="icon" type="image/png" sizes="32x32" href="home/assets/images/logo.png">
    <link rel="icon" type="image/png" sizes="16x16" href="home/assets/images/logo.png">
    <link rel="shortcut icon" type="image/x-icon" href="home/assets/images/logo.png">
    <meta name="msapplication-TileImage" content="home/assets/images/logo.png">

    <!-- Font Icon -->
    <link rel="stylesheet" href="fonts/material-icon/css/material-design-iconic-font.min.css">

    <!-- Main css -->
    <link rel="stylesheet" href="css/style.css">
</head>

<body>

    <div class="main">



        <!-- forgot password  Form -->
        <section class="forgot" id="forgot">
            <div class="container">
                <div class="signin-content">
                    <div class="signin-image">
                        <figure><img src="images/signin-image.jpg" alt="sign up image"></figure>

                    </div>

                    <div class="signin-form">
                        <h2 class="form-title">Forgot Password</h2>
                        <form method=" POST" class="register-form" id="login-form">
                            <div class="form-group">
                                <label for="your_name"><i class="zmdi zmdi-email material-icons-name"></i></label>
                                <input type="email" name="femail" id="femail" placeholder="Your Email" />
                            </div>
                            <div class="form-group">
                                <b>
                                    <p class="signup-image-link" style="color: #be1e2d" id="fmsg"></p>
                                </b>
                            </div>

                            <div class="form-group form-button">
                                <input type="button" name="fsub" id="fsub" class="form-submit" value="Recover" />
                            </div>
                            <a href="./" style="text-decoration: none" class="signup-image-link mr-0 ml-0">
                                My DashBoard</a>
                            &nbsp;&nbsp;
                            |
                            &nbsp;&nbsp; <a href="./welcome" style="text-decoration: none"
                                class="signup-image-link mr-0 ml-0">Create
                                account</a>
                        </form>
                        <!--<div class="social-login">
                            <span class="social-label">Or login with</span>
                           <ul class="socials">
                                <li><a href="#"><i class="display-flex-center zmdi zmdi-facebook"></i></a></li>
                                <li><a href="#"><i class="display-flex-center zmdi zmdi-twitter"></i></a></li>
                                <li><a href="#"><i class="display-flex-center zmdi zmdi-google"></i></a></li>
                            </ul>
                        </div>-->
                    </div>
                </div>
            </div>
        </section>

        <!-- verify notifications --->
        <section class="verify" id="verify" hidden>
            <div class="container">
                <div class="signin-content">
                    <div class="signin-image">
                        <figure><img src="images/signin-image.jpg" alt="sign up image"></figure>

                    </div>

                    <div class="signin-form">
                        <h2 class="form-title">RECOVER PASSWORD</h2>

                        <p>We've sent a link to your registered email.</p>
                        <p>Kindly check your inbox and spam folders to recover your password</p>
                    </div>
                </div>
            </div>
        </section>

    </div>


</body>
<!-- JS -->
<script src="vendor/jquery/jquery.min.js"></script>
<script src="js/main.js"></script>
<script src="js/ajax.js"></script>

<!-- hide elements -->
<script>
function verify() {
    document.getElementById('forgot').style.display = 'none';
    document.getElementById('verify').style.display = 'block';
}
</script>

</html>